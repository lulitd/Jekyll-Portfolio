function resizeWorkCards(){
    $( '.work' ).each(function(index) {
      var imgHeight = $(this).find('img').height();
      var infoContainer= $(this).find('.info');
      var infoPadding =   infoContainer.innerHeight() - infoContainer.height();
      infoContainer.height(imgHeight-infoPadding);

      var paragraphContainer= $(this).find('p');
      var headerContainer = $(this).find('h3');
      var hHeight= headerContainer.outerHeight();
      var pHeight = paragraphContainer.outerHeight();

  if ((hHeight + pHeight) > infoContainer.height()){
        while ( (hHeight + pHeight) > infoContainer.height()) {

     paragraphContainer.css("font-size","-=1" );
     headerContainer.css("font-size","-=2" );

     pHeight = paragraphContainer.outerHeight();
     hHeight= headerContainer.outerHeight();
 }
}
else if ((hHeight + pHeight) < infoContainer.height() * 0.7){
 while ( (hHeight + pHeight) < infoContainer.height()*0.7) {

   paragraphContainer.css("font-size","+=1" );
   headerContainer.css("font-size","+=1" );
